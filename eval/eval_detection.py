...
class ANETdetection(object):
    ...
    def __init__(self,
        ...
        if self.verbose:
            ...
            logger.info('\tFixed threshold for tiou score: {}'.format(
    ...
    def _import_ground_truth(self, ground_truth_filename):
        ...
        class_dict = {int(k): v for k, v in data['class-list'].items()}
        activity_index = dict(zip(class_dict.values(), class_dict.keys()))
        ...
        ground_truth = ground_truth.sort_values(by="video-id",
                                                ignore_index=True)
        if self.verbose:
            logger.info(activity_index)
    ...
    def _import_prediction(self, prediction):
        ...
        # prediction already in dict format
        if isinstance(prediction, dict):
            data = prediction
        else:
            # load from file
            with open(prediction, 'r') as fobj:
                data = json.load(fobj)
        # Checking format...
        ...
        prediction = prediction.sort_values(by="video-id", ignore_index=True)
        return prediction
    ...
    def _get_predictions_with_label(self, prediction_by_label, label_name,
        ...
        try:
            ...
            if self.verbose:
                logger.info(
                    'Warning: No predictions of label \'%s\' were provdied.' %
                    label_name)
            return pd.DataFrame()
    ...
    def wrapper_compute_average_precision(self):
        ...
        # #! DEBUG
        # results = []
        # self.activity_index = {"Brushing hair": 0}

        # ap = np.zeros((len(self.tiou_thresholds), len(self.activity_index)))

        # for label_name, cidx in self.activity_index.items():
        #     _res = compute_average_precision_detection(
        #         ground_truth=ground_truth_by_label.get_group(cidx).reset_index(
        #             drop=True),
        #         prediction=self._get_predictions_with_label(
        #             prediction_by_label, label_name, cidx),
    ...
    def evaluate(self, details=True):
        ...
        # if details:
        #     logger.info()
        #     logger.info(Fore.LIGHTGREEN_EX + f"Class wise scores:")
        #     logger.info(df_cls)
        # logger.info()

        df_cls = pd.DataFrame(
            data=self.ap.transpose() * 100,
            columns=self.tiou_thresholds,
            index=[cidx for _, cidx in self.activity_index.items()])
        df_cls.insert(0, 'Name', [
            f'{label_name:>20s}'
            for label_name, _ in self.activity_index.items()
        ])
        df_cls.loc['mean'] = df_cls.mean()
        df_cls['mean'] = df_cls.iloc[:, 1:].mean(axis=1)

        return self.mAP, self.average_mAP, df_cls
